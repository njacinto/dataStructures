...
public class IntHeapSortMultiArray {    
    public static final IntHeapSortMultiArray INSTANCE = new IntHeapSortMultiArray();
    
    public static IntHeapSortMultiArray getInstance(){
        return INSTANCE;
    }
...
//    @Override
    public int[] sort(int[] data, int from, int to, IntComparator comparator, Object[] ... otherData) {
        if(from>=to){
        ...
        while (end > begin) {
            ...
            data[end] = data[begin];
            data[begin] = tmpVal;
            for(Object[] arr : otherData){
                final Object tmp = arr[end];
                arr[end] = arr[begin];
                arr[0] = tmp;
            }
            siftDown(data, begin, end - 1, comparator, otherData);
            end--;
        }
        return data;
    }

    private void siftDown(int[] data, int start, int end, IntComparator comparator, Object[] ... otherData) {
        int root = start;
        int child;
        int tmpVal;
