...
public class IntHashTreeMap implements IntMap {
...
//    protected final Iterable<Entry> ENTRY_ITERABLE = new HashIterable();
    ...
    public IntHashTreeMap() {
        this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR);
    }
    
    public IntHashTreeMap(int capacity) {
        this(capacity, DEFAULT_LOAD_FACTOR);
    }
    
    @SuppressWarnings("OverridableMethodCallInConstructor")
    public IntHashTreeMap(int capacity, float loadFactor) {
    ...
    public IntHashTreeMap setLoadFactor(float loadFactor) {
     ...
     * @return  
     */
    public IntHashTreeMap setComparator(IntComparator comparator) {
    ...
    @Override
    public void putAll(IntMap c) {
        for(IntMap.Entry entry : c.entrySet()){
            putNode(entry.getKey(), entry.getValue());
        }
    }

    @Override
    public void putAll(Map<? extends Integer, ? extends Integer> c) {
        for(Map.Entry<? extends Integer,? extends Integer> entry : c.entrySet()){
            putNode(entry.getKey(), entry.getValue());
        }
    }
    ...
    public int[] valuesToArray(int fromIndex, int toIndex) {
    ...
    public PrimitiveIterator.OfInt iteratorValues() {
     ...
     * @return 
     */
    private int putNode(int key, int value) {
    ...
    private MapTreeNode getNodeOrParent(MapTreeNode root, int value){
    ...
    protected class MapTreeNode implements IntMap.Entry {
        private int key;
        private int value;
        ...
        MapTreeNode(int key, int value) {
            this.key = key;
            this.value = value;
        }

        MapTreeNode(int key, int value, boolean isRed) {
            this.key = key;
            this.value = value;
            this.red = isRed;
        }

        MapTreeNode(int key, int value, MapTreeNode parent) {
            this.key = key;
            this.value = value;
            this.parent = parent;
        }
         ...
         * of the tree.
         * To change the value use the IntHashTreeMap.put operation.
         * 
         * @param value
         * @return 
...
//            return tmp;
        ...
        public int compareTo(int otherValue){
            return IntHashTreeMap.this.comparator.compare(value, otherValue);
        }
    ...
    protected class TreeIterable implements Iterable<IntMap.Entry> {

        @Override
        public Iterator<IntMap.Entry> iterator() {
            return new TreeIterator(IntHashTreeMap.this.concurrentModification, 
                    IntHashTreeMap.this.comparator,  getSmallestEntry(root));
        }
        
    }
    
    protected class TreeIterator implements Iterator<IntMap.Entry> {
        ...
        TreeIterator(int concurrentModification, IntComparator comparator, MapTreeNode start){
        ...
        public MapTreeNode next() {
            ...
            if(this.concurrentModificationMark!=IntHashTreeMap.this.concurrentModification){
        ...
        public void remove() {
            ...
            if(prev!=null){
                MapTreeNode node = IntHashTreeMap.this.removeNodeFromTree(prev);
                prev = null;
                this.concurrentModificationMark = IntHashTreeMap.this.concurrentModification;
