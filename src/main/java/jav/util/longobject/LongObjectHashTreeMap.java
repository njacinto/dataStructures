...
import jav.util.longobject.LongObjectHashTreeMap.MapTreeNode;
import jav.util.longobject.LongObjectHashTreeMap.TreeIterator;
...
public class LongObjectHashTreeMap<V> implements LongObjectMap<V> {
    ...
    //
    protected final Iterable<LongObjectMap.Entry<V>> treeIterable = new TreeIterable();
    //
...
//    protected final Iterable<Entry> ENTRY_ITERABLE = new HashIterable();
    ...
    public LongObjectHashTreeMap() {
        this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR);
    }
    
    public LongObjectHashTreeMap(int capacity) {
        this(capacity, DEFAULT_LOAD_FACTOR);
    }
    
    @SuppressWarnings({"OverridableMethodCallInConstructor", "unchecked"})
    public LongObjectHashTreeMap(int capacity, float loadFactor) {
    ...
    public LongObjectHashTreeMap setLoadFactor(float loadFactor) {
     ...
     * @return  
     */
    public LongObjectHashTreeMap setComparator(Comparator<V> comparator) {
    ...
    @Override
    public void putAll(LongObjectMap<V> c) {
        for(LongObjectMap.Entry<V> entry : c.entrySet()){
            putNode(entry.getKey(), entry.getValue());
        }
    }

    @Override
    public void putAll(Map<? extends Long, ? extends V> c) {
        for(Map.Entry<? extends Long,? extends V> entry : c.entrySet()){
            putNode(entry.getKey(), entry.getValue());
        }
    }
    ...
    public Set<Entry<V>> entrySet() {
        return new LongObjectMapUtil.EntrySet<V>(treeIterable, this);
    }

    @Override
    public LongSet longKeySet() {
        return new LongObjectMapUtil.KeyLongObjectSet<V>(treeIterable, this);
    }

    @Override
    public Collection<V> values() {
        return new LongObjectMapUtil.ValueLongObjectCollection<>(treeIterable, this);
    }
     ...
     * @return 
     */
    private V putNode(long key, V value) {
    ...
    protected class MapTreeNode implements LongObjectMap.Entry<V> {
        private long key;
        private V value;
        ...
        MapTreeNode(long key, V value) {
            this.key = key;
            this.value = value;
        }

        MapTreeNode(long key, V value, boolean isRed) {
            this.key = key;
            this.value = value;
            this.red = isRed;
        }

        MapTreeNode(long key, V value, MapTreeNode parent) {
            this.key = key;
            this.value = value;
            this.parent = parent;
        }
         ...
         * of the tree.
         * To change the value use the LongObjectHashTreeMap.put operation.
         * 
         * @param value
         * @return 
...
//            return tmp;
        ...
        public int compareTo(V otherValue){
            return LongObjectHashTreeMap.this.comparator.compare(value, otherValue);
        }
    ...
    protected class TreeIterable implements Iterable<LongObjectMap.Entry<V>> {

        @Override
        public Iterator<LongObjectMap.Entry<V>> iterator() {
            return new TreeIterator(LongObjectHashTreeMap.this.concurrentModification, getSmallestEntry(root));
        }
        
    }
    
    protected class TreeIterator implements Iterator<LongObjectMap.Entry<V>> {
        ...
        public MapTreeNode next() {
            ...
            if(this.concurrentModificationMark!=LongObjectHashTreeMap.this.concurrentModification){
        ...
        public void remove() {
            ...
            if(prev!=null){
                MapTreeNode node = LongObjectHashTreeMap.this.removeNodeFromTree(prev);
                prev = null;
                this.concurrentModificationMark = LongObjectHashTreeMap.this.concurrentModification;
