...
public class IntConcurrentMapIteratorTest extends IntMapEntryIteratorTest {

    @Override
    protected Iterator<IntMap.Entry> iterator() {
        return new IntConcurrentHashMap().iterator();
    }

    @Override
    protected Iterator<IntMap.Entry> iterator(int[]... nums) {
        IntConcurrentHashMap map = new IntConcurrentHashMap(nums.length);
