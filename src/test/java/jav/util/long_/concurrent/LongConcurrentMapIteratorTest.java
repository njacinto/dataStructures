...
public class LongConcurrentMapIteratorTest extends LongMapEntryIteratorTest {

    @Override
    protected Iterator<LongMap.Entry> iterator() {
        return new LongConcurrentHashMap().iterator();
    }

    @Override
    protected Iterator<LongMap.Entry> iterator(long[]... nums) {
        LongConcurrentHashMap map = new LongConcurrentHashMap(nums.length);
