 ...
 * @author njacinto
 */
public final class LongMapConcurrencyTestUtil {
    private static final Logger log = Logger.getLogger(LongMapConcurrencyTestUtil.class.getName());

    private LongMapConcurrencyTestUtil() {
    }
    ...
    public static boolean testLong(LongConcurrentHashMapAbstract map, int numElements, int numOfThreads, 
    ...
    public static boolean checkMap(LongMap map, int numElements, int numOfThreads){
        final int numOfElementsPerThread = numElements/numOfThreads;
    ...
    private static class DataOperationsRunnable implements Callable<Boolean> {
        ...
        public DataOperationsRunnable(LongMap map, int numOfIteractions, int numElements, int startIndex) {
            this.map = map;
        ...
        public Boolean call() {
            try {
                for(int j=0; j<numOfIteractions; j++){
                    for(long i=startIndex; i<startIndex+numElements; i++){
                        map.put(i,i);
                    }
                    for(long i=(startIndex+numElements+numElements-1); i>=startIndex+numElements; i--){
                        map.put(i, i);
                    }
                    for(long i=(startIndex+numElements+numElements-1); i>=startIndex+numElements; i--){
                        map.remove(i);
    ...
    private static class IteratorTestRunnable implements Callable<Boolean> {
        ...
        public IteratorTestRunnable(Iterable<LongMap.Entry> iterable, boolean throwsConcurrentModification) {
    ...
    private static class ArrayTestRunnable implements Callable<Boolean> {
        private final LongConcurrentHashMapAbstract map;

        public ArrayTestRunnable(LongConcurrentHashMapAbstract map) {
            this.map = map;
        }
